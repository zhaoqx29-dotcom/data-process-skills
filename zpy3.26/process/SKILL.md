---
name: process
description:数据处理子 skill，负责执行数据处理、生成评价报告。当主 skill 需要执行数据处理操作时调用此 skill。输入：df（原始数据）、plan（调用链表）输出：df_result（处理后数据）、evaluation_report
---
...
from process import chain_execute, generate_evaluation_report
