...
| `transform_diff2(df, columns)` | 二阶差分 | 消除趋势趋势 |
...
| 函数 | 说明 | 典型场景 |
|------|------|----------|
| `outlier_3sigma_detect(df, columns)` | 3-Sigma检测 | 正态分布数据 |
| `outlier_iqr_detect(df, columns, k)` | IQR检测 | 非正态分布 |
| `outlier_zscore_detect(df, columns)` | Z-score检测 | 标准化后检测 |
| `outlier_moving_std_detect(df, columns)` | 移动标准差检测 | 时间序列 |
| `outlier_dbscan_detect(df, columns)` | DBSCAN检测 | 复杂分布 |

### 处理函数
| 函数 | 说明 | 典型场景 |
|------|------|----------|
| `outlier_3sigma_clip(df, columns)` | 3-Sigma裁剪 | 正态分布数据 |
| `outlier_iqr_clip(df, columns, k)` | IQR裁剪 | 非正态分布 |
| `outlier_zscore_clip(df, columns)` | Z-score裁剪 | 快速处理 |
| `outlier_moving_std_clip(df, columns)` | 移动标准差裁剪 | 时间序列 |
| `outlier_dbscan_remove(df, columns)` | DBSCAN移除 | 复杂分布 |
